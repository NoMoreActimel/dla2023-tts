...
def main(config):
    ...
    model = module_model.FastSpeech2(config.config)
    logger.info(model)
    ...
    run_inference(
        ...
        indices=[30, 40, 50],
        waveglow_path=config["data"]["val"]["waveglow_path"],
        dataset_type="val",
        inference_path="data/datasets/ljspeech/inference",
        duration_coeffs=[0.8, 1.0, 1.2],
        pitch_coeffs=[0.8, 1.0, 1.2],
        energy_coeffs=[0.8, 1.0, 1.2],
        epoch=None
    )
